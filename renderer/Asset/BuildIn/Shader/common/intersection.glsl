	...
	texture2D hiz, sampler minSampler) 	//Hiz的第0级没做复制，还在原深度缓冲里
{
	...
	if (ProjectSphere(center, radius, view, z_near, proj[0][0], proj[1][1], aabb)) 
	{
		...
		float depth;
		depth = textureLod(sampler2D(hiz, minSampler), uv, level).r;
		depth = max(depth, textureLod(sampler2D(hiz, minSampler), vec2(aabb.x, aabb.y), level).r);
		depth = max(depth, textureLod(sampler2D(hiz, minSampler), vec2(aabb.x, aabb.w), level).r);
		depth = max(depth, textureLod(sampler2D(hiz, minSampler), vec2(aabb.z, aabb.y), level).r);
		depth = max(depth, textureLod(sampler2D(hiz, minSampler), vec2(aabb.z, aabb.w), level).r);
			
		vec3 dir 				= normalize(cameraPos - center);	//投影世界空间球面离相机最近的点到裁剪空间，比较深度
		float distance 			= length(cameraPos - center);		//距离小于包围盒半径的也直接当可见
		vec4 sceenSpaceNear 	= proj * view * vec4(center + dir * radius, 1.0);
		float depthSphere 		= sceenSpaceNear.z / sceenSpaceNear.w;

		visible = (distance <= radius || depthSphere <= depth);
	}
